extends SceneTree
# [VP] night_strip visual pass (lot v0.18.1). Staged into the lux project by
# tools/night_strip.ps1. Instances the three storefronts at their SITE
# transforms (keep in sync with specs/night_strip.site.json), instances the
# Zoo fixtures GLB (world-space site build), bakes the merged site manifest
# (streetlights included), binds emissives, grades Blue Hour, and shoots the
# reel-comparison set. Windowed: rendering required.
...
const STORES: Array = [
	{"glb": "night_deli.glb", "pos": Vector3(-34, 0, -21)},
	{"glb": "night_pawn.glb", "pos": Vector3(0, 0, -9)},
	{"glb": "night_auto.glb", "pos": Vector3(28, 0, -11)},
]
...
func _main() -> void:
	...
	stage.name = "NightStrip"
	root.add_child(stage)

	for s in STORES:
		var entry: Dictionary = s
		var ps: PackedScene = load(DIRP + "/" + String(entry["glb"])) as PackedScene
		if ps == null:
			_log("WARN: missing " + String(entry["glb"]))
			continue
	...
	_log("night strip visual pass complete - " + str(SHOT_LIST.size()) + " shots")
	quit(0)
